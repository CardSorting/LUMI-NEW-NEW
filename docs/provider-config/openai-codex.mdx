...
description: "Connect your OpenAI account to LUMI via OAuth for seamless access to OpenAI models."
---

OpenAI Codex connects your ChatGPT account to LUMI with OAuth. LUMI refreshes the account's available model list in Settings and sends the full conversation context with each turn, so a follow-up does not depend on a stored server-side response.
...
<Steps>
  ...
  <Step title="Sign In with OpenAI">
    Click **"Connect ChatGPT"**. This opens a browser window for authentication.
  </Step>
  <Step title="Authorize LUMI">
    In the browser window that opens:
    1. Sign in to your OpenAI account (if not already signed in)
    2. Review the permissions LUMI is requesting
    3. Click **"Authorize"** to grant access
  </Step>
  <Step title="Select Your Model">
    Once authorized, LUMI loads the models available to your account. Choose a model from the **"Model"** dropdown; use **"Refresh account models"** after changing account access.
  </Step>
</Steps>
...
To disconnect your OpenAI account from LUMI:

1. Open LUMI Settings
...
OAuth tokens are automatically refreshed by LUMI. If you encounter authentication errors:
...
LUMI stores the OAuth credentials in its local secret storage so it can refresh the session. They are used to authenticate Codex requests and are not shown in the model catalog.

When using ChatGPT Subscription (`openai-codex`), LUMI accesses models through your OpenAI/ChatGPT subscription. There is no separate per-token API billing for this provider; usage and limits are governed by your subscription plan.
