...
LOGGED_OUT_HINT = ("Claude Code is installed but has no usable login for the user running LUMI. Run `claude auth login` "
                   "as that user, then confirm the account with `claude auth status` and try again.")
...
def discover_models(command=None, env=None, timeout=40):
    """The account's live picker as ``[{id, label, note, upstream_requests}]`` in LUMI route ids,
    ...
    try:
        ...
        handshake = json.dumps({"type": "control_request", "request_id": "lumi-picker",
