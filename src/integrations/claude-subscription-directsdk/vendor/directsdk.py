...
class ClaudeCodeLoggedOut(RuntimeError):
    """Claude Code refused before any upstream request because LUMI has no usable login."""
...
def request_body(kwargs):
    ...
    for key in ('temperature', 'top_p'):
        ...
        # Subscription models reject sampling controls, including LUMI's title
...
class Client:
    def __init__(self, command=None, args=None, env=None, timeout=180, **_):
        # Keep the client shape expected by the original transport adapter.
    ...
    def create(self, **kwargs):
        # Async callers may await create directly; it returns the same response shape.
        try:
    ...
    def _run(self, request, kwargs, body, manifest, names, system, frames):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix='claude-directsdk-', ignore_cleanup_errors=True) as tmp:
                ...
                mcp = {'mcpServers': {'lumi': {'command': sys.executable, 'args': [str(Path(__file__).with_name('inert_mcp.py')), str(root / 'tools.json')]}}}
                ...
                # LUMI owns context budgets; native's replayed reminder invalidates cache history.
                ...
                # per-argument/environment-string limit for full LUMI tool schemas.
                ...
                if native_error and not native_failure_handled:
                    if native_error_code == 'authentication_failed' and not admission.used:
                        # No usable login in LUMI's environment: Claude Code refuses before sending a model request.
