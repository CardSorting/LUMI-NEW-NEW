		...
		switch (apiProvider) {
			...
			case "vertex":
				if (!apiConfiguration.vertexApiKey) {
					return "You must provide a valid Google Cloud API Key."
				}
				break
			case "gemini":
...
export function validateModelId(
	...
	dietcodeModels?: Record<string, ModelInfo>,
): string | undefined {
	if (apiConfiguration) {
		const { apiProvider, openRouterModelId, dietcodeModelId } = getModeSpecificFields(apiConfiguration, currentMode)
		switch (apiProvider) {
			case "openrouter":
				...
				break
			case "dietcode":
				const dietcodeResolvedModelId = dietcodeModelId || openRouterDefaultModelId
				if (!dietcodeResolvedModelId) {
					return "You must provide a model ID."
				}
				if (dietcodeModels && !Object.keys(dietcodeModels).includes(dietcodeResolvedModelId)) {
					return "The model ID you provided is not available. Please choose a different model."
				}
				break
		}
